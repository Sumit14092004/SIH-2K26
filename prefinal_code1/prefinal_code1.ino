...
bool tryInitBMP(Adafruit_BMP280 &b, TwoWire &bus, uint8_t addr) {
  ...
  // Try begin with detected chip ID, or known IDs (BMP280=0x58, BME280=0x60, clones=0x56/0x57)
...
float readUltrasonic() {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    delay(10); // Inter-ping settle time to let acoustic reverberation attenuate
  }

  if (count == 0) return (filteredDist > 0 ? filteredDist : -1.0);
  ...
  if (filteredDist < 0) {
    ...
    // Slew rate filter: reject transient ghost spikes > 35cm unless confirmed
    if (fabs(median - filteredDist) > 35.0) {
      anomalyCount++;
      if (anomalyCount >= 2) {
        filteredDist = median;
        anomalyCount = 0;
      ...
      filteredDist = 0.60f * filteredDist + 0.40f * median;
      anomalyCount = 0;
    }
  }
...
void setup() {
  ...
  Serial.println("✅ [8/9] HC-SR04 Flood Radar   : ONLINE (Pins 5 & 18)");
...
void loop() {
  ...
  // 2. BMP280 Barometer
  static float last_valid_pres = 1001.2f;
  static float last_valid_alt = 110.0f;
  float pres = last_valid_pres;
  float alt = last_valid_alt;

  if (bmp_ready && active_bmp) {
    float raw_p = active_bmp->readPressure() / 100.0F;
    if (!isnan(raw_p) && raw_p >= 850.0f && raw_p <= 1100.0f) {
      last_valid_pres = raw_p;
      last_valid_alt  = active_bmp->readAltitude(1013.25);
    }
    pres = last_valid_pres;
    alt  = last_valid_alt;
  ...
  // 8. HC-SR04 Ultrasonic (GLITCH-FREE BASELINE HOLD)
  ...
  if (raw_dist > 0.0f && raw_dist <= 400.0f) {
    ...
    dist = raw_dist;
    Serial.printf("🌊 [8] Ultrasonic: Distance = %5.1f cm [ONLINE]\n", dist);
  } else {
    dist = last_valid_dist; // Holds previous baseline; prevents fake -1.0 cm flood spikes
    Serial.printf("🌊 [8] Ultrasonic: NO ECHO / Beam Timeout (Holding baseline %.1f cm)\n", dist);
  }
  ...
  // ========================================================
  // 🧠 QUALCOMM ON-DEVICE TINYML (MULTI-HAZARD INFERENCE)
  // ========================================================
  // Seismic Deadband Calibration: Discard normal desk handling tilt & vibration
  ...
  if (esp_now_ready) {
    ...
    // Case-insensitive alert verification (Fixes uppercase NORMAL heartbeat issue)
